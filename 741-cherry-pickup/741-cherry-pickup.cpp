class Solution {
    ...
    int rec(int i1, int j1, int i2, vector<vector<int>>& grid) {
        int n = grid.size();
        int j2 = (i1 + j1) - i2;
        ...
        int a = rec(i1+1, j1, i2+1, grid);
        int b = rec(i1+1, j1, i2, grid);
        int c = rec(i1, j1+1, i2+1, grid);
        int d = rec(i1, j1+1, i2, grid);

        int mx = max({a, b, c, d});
        
        mx += ((i1 == i2 and j1 == j2) ? grid[i1][j1] : grid[i1][j1] + grid[i2][j2]);

        return dp[i1][j1][i2] = mx;
    }
